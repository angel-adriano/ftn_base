...
class PosOrder(models.Model):
    ...
    forma_pago_id  = fields.Many2one('catalogo.forma.pago', string='Forma de pago', compute='_find_forma_pago')
    ...
    uso_cfdi_id  = fields.Many2one('catalogo.uso.cfdi', string='Uso CFDI (cliente)', compute='_find_uso_cfdi')
    ...
    def _find_forma_pago(self):
         for order in self:
            if order.forma_pago:
                order.forma_pago_id = self.env['catalogo.forma.pago'].sudo().search([('code','=',order.forma_pago)], limit=1)
            else:
                order.forma_pago_id = None

    @api.onchange('uso_cfdi')
    def _find_uso_cfdi(self):
        for order in self:
            if order.uso_cfdi:
                order.uso_cfdi_id =  self.env['catalogo.uso.cfdi'].sudo().search([('code','=',order.uso_cfdi)], limit=1)
            else:
                order.uso_cfdi_id = None
