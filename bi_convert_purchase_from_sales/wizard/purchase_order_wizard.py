...
class createpurchaseorder(models.TransientModel):
    ...
    existing_order_id = fields.Many2one('purchase.order', string='Existing Purchase Order',
                                       domain="[('partner_id', '=', partner_id), ('state', '=', 'draft')]")
    has_draft_orders = fields.Boolean(string='Has Draft Orders', compute='_compute_has_draft_orders', store=False)

    @api.onchange('partner_id')
    def _compute_has_draft_orders(self):
        """Check if the selected vendor has any draft purchase orders"""
        for record in self:
            if record.partner_id:
                ...
                record.has_draft_orders = len(draft_orders) > 0
            else:
                record.has_draft_orders = False

    @api.onchange('partner_id')
    def _onchange_partner_id(self):
        """Clear existing_order_id when partner changes"""
        if self.partner_id:
            self.existing_order_id = False
    ...
    def action_create_purchase_order(self):
        """Create a Purchase Order from sale or add lines to existing order"""
        self.ensure_one()
        so = self.env['sale.order'].browse(self._context.get('active_id'))
        sale_order_name = so.name
        company_id = self.env.company

        # Check if we should add to existing order or create new one
        if self.existing_order_id:
            purchase_order = self.existing_order_id
            # Update origin to include this sale order
            if purchase_order.origin:
                origins = purchase_order.origin.split(', ')
                if sale_order_name not in origins:
                    purchase_order.origin = purchase_order.origin + ', ' + sale_order_name
            else:
                purchase_order.origin = sale_order_name
        else:
        ...
        if self.existing_order_id:
            message = "Lines added to existing Purchase Order " + purchase_order.name + " from Sale Order " + sale_order_name
        else:
