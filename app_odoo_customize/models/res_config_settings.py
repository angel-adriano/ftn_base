...
class ResConfigSettings(models.TransientModel):
    ...
    company_to_clean_id = fields.Many2one('res.company', string='Company to Clean')
    ...
    def get_values(self):
        ...
        company_id = ir_config.get_param('company_to_clean_id')
        res.update(
            company_to_clean_id=int(company_id) if company_id else False
        )
    ...
    def set_values(self):
        ...
        ir_config.set_param("company_to_clean_id", self.company_to_clean_id.id if self.company_to_clean_id else "")
    ...
    # 清数据，o=对象, s=序列
    def _remove_app_data(self, o, s=[], company=None):
        for model_name in o:
            try:
                if not self.env['ir.model']._get(model_name):
                    continue
            except Exception as e:
                _logger.warning('Skipping unknown model: %s (%s)', model_name, e)
                continue
        ...
        for prefix in s:
            ...
            try:
                ...
                _logger.warning('Failed to reset sequences for prefix %s: %s', prefix, e)
    ...
    def remove_sales(self):
        ...
        if not self.company_to_clean_id:
            raise UserError(_('Please select a company to clean.'))
        return self._remove_app_data(to_removes, seqs, company=self.company_to_clean_id)
