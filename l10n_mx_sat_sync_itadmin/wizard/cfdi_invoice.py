...
class CfdiInvoiceAttachment(models.TransientModel):
    ...
    def import_customer_invoice(self, file_content, journal=False):
        ...
        invoice_vals = {
           ...
           'uso_cfdi_id': self.env['catalogo.uso.cfdi'].sudo().search([('code','=',data.get('@UsoCFDI'))]).id,
            'factura_cfdi': True,
...
##################################################################################
    ...
    def get_or_create_product(self, default_code, product_name, clave_unidad, unit_price, clave_producto, sale_ok=True,
        ...
        product_types = dict(product_obj._fields.get('type')._description_selection(product_obj.env))
        product_type_default = self.env['ir.config_parameter'].sudo().get_param(
            'l10n_mx_sat_sync_itadmin.product_type_default')
