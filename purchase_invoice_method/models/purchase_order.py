...
class PurchaseOrder(models.Model):
    ...
    advance_purchase = fields.Boolean(string='Pago anticipado')

    @api.onchange('advance_purchase')
    def set_to_invoice(self):
        if self.advance_purchase:
            self.invoice_method = 'purchase'
        if not self.advance_purchase:
            self.invoice_method = 'receive'
