...
class AccountMove(models.Model):
    ...
    def action_get_xml_invoice_sanmex(self):
        xml_ids =[]
        for invoice in self:
            try:
                ...
                if attachment:
                    ...
                    if invoice_id.state == 'posted' and invoice_id.estado_factura == 'factura_correcta':
                        #xml_ids.append((invoice.id, invoice.name))
                        #raise UserError(_('Xml encontrado ' + str(invoice_id.company_id)))
                        if attachment.datas:
                            xml_ids.append((invoice.id, invoice.name))
                        ...
                        #raise UserError(_('Xml encontrado ' + str(cargar_values)))
                        ...
                        #xml_ids.append((invoice.id, invoice.name))
                        #raise UserError(_('Xml encontrado ' + str()))
            except Exception as e:
                _logger.error(str(e))
                pass
        return True
        #raise UserError(_('Xml encontrado ' + str(xml_ids)))